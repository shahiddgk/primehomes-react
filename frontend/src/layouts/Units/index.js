...
function Units() {
  ...
  const editUnit = (row) => {
    ...
    setSelectedOwner({
      id: row?.owner?._id,
      value: `${row?.owner?.code} - ${row?.owner?.title} ${row?.owner?.firstName} ${row?.owner?.middleName} ${row?.owner?.lastName}`
    })
  }
      ...
      try{
        ...
        setUnitsData(res.data.data.map(x => {
          ...
          return{
            ...
            owner: `${x.owner.code} - ${x.owner.title} ${x.owner.firstName} ${x.owner.middleName} ${x.owner.lastName}`,
  ...
  const createUnitHandler = async () => {
    try{
      ...
      setUnitsData([...unitsData, {
        ...
        owner: People.filter(p => p._id === x.owner).map(p => `${p.code} - ${p.title} ${p.firstName} ${p.middleName} ${p.lastName}`),
  ...
  const updateUnitHandler = async () => {
    try{
      ...
      setUnitsData(unitsData.map(x =>  x._id === modalValue._id ? {
        ...
        owner: People.filter(p => p._id === data.owner).map(p => `${p.code} - ${p.title} ${p.firstName} ${p.middleName} ${p.lastName}`),      
        isParking: x.isParking ? 'Yes' : 'No',
        isFullyPaid: x.isFullyPaid ? 'Yes' : 'No',
    } : x ))
    }catch(err){
