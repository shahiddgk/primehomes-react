...
export default function Representatives() { 
  ...
  const handleEdit = (row) => {
    ...
    setSelectedRepresentativeId(row._id);
  ...
  const handleDelete = (row) => {
    Swal.fire({
      title: `Are You Sure To Delete The Representative?
            <i> ${row.name}</i>`,
      ...
      if (result.isConfirmed) {
        axios.delete(`representatives/${row._id}`) 
          .then(async (response) => {
            console.log(response.data.data);
            Swal.fire('Deleted!', `${response.data.message}`, 'success');
            try {
              const res = await axios.get('representatives/represtators'); 
              setRepresentatives(
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('representatives/represtators'); 
      console.log(response.data.data);
      setRepresentatives(
  ...
  const handleUpdate = () => {
    ...
    axios
      .patch(`representatives/${selectedRepresentativeId}`, formData, {
        headers: {
      ...
      .catch((error) => {
        ...
        Swal.fire({
          title: 'Error!',
          text: 'An error occurred while updating the representative.',
          icon: 'error',
        ...
        <Grid container spacing={6}>
          ...
          <Grid item xs={6} md={3}>
            <MDButton
              ...
              onClick={createRepresentativeBtn}
            >
          ...
          {/* Pop-up Model for creating/updating Representatives */}
          <Modal
            size="lg"
            show={modal.show}
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title id="contained-modal-title-vcenter">
                {modal.forEdit ? 'Edit Representative' : 'Create New Representative'}
              </Modal.Title>
            </Modal.Header>
            ...
            <Modal.Footer>
              ...
              >
                <Create /> &nbsp;&nbsp;
                {modal.forEdit ? 'Update Representative' : 'Create Representative'}
              </MDButton>
