...
const getAll = async () => {
    return await UnitesModel.find({}).populate([{path: 'building', model:'Buildings', select: {name:1, code:1}}, {path: 'owner', model:'People', select: {code:1, title: 1, firstName: 1, middleName: 1, lastName: 1}}]).sort({updatedAt: -1})
  };
